...
public class SpreoFromToListAdapter extends ArrayAdapter<IPoi> {
    Context context;
    List<IPoi> pois;
    ...
    public SpreoFromToListAdapter(Context context, int resource, List<IPoi> objects, SpreoSearchItem type) {
...
//    }
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        SpreoFromToListAdapter.ViewHolder holder = null;
        ...
        if (convertView == null) {
            ...
            // } else
            if (pois.size() == 1) {
                convertView = mInflater.inflate(R.layout.spreo_fromto_list_item, null);
                try {
                    convertView.setBackgroundResource(R.drawable.layout_bg_selector_single);
                } catch (Exception e) {
                    e.printStackTrace();
                ...
                convertView = mInflater.inflate(R.layout.spreo_fromto_list_item, null);
            holder = new SpreoFromToListAdapter.ViewHolder();
            ...
            holder = (SpreoFromToListAdapter.ViewHolder) convertView.getTag();
    ...
    public Filter getFilter() {
        if (filter == null)
            filter = new SpreoFromToListAdapter.AppFilter<IPoi>(pois);
        return filter;
    }

    private void setBaseStateForPoiFacilityView(@NonNull SpreoFromToListAdapter.ViewHolder holder){

        holder.poiFacility.setText("");
        holder.poiFacility.setVisibility(View.VISIBLE);
    }

    private void setFloorAndFacilityIdInViewIfPossible(IPoi poi, SpreoFromToListAdapter.ViewHolder holder) {
